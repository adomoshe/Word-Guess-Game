...
var movieList = ["Pulp Fiction", "Inglorious Basterds", "Reservoir Dogs", "The Hateful Eight", "Kill Bill", "Jackie Brown", "Django Unchained"],
    ...
    moviePick = movieList[Math.floor(Math.random() * movieList.length)],
    ...
    numberOfLetters = moviePickLowerCase.length,
...
// Loop used to display the number of characters in the moviePick with "_" so the player can see how many characters are in the word
...
document.onkeydown = function game(){
    ...
    var collectPositions = [];
        ...
        wordBeingGuessed = wordBeingGuessed.substr(0, collectPositions[0] * 2) + userGuess + " " + wordBeingGuessed.substr((collectPositions[0] * 2) + 2, wordBeingGuessed.length * 2);
    ...
    if (moviePickLowerCase === wordBeingGuessed.replace((/\s/g), '')) {
        wins++;
    }
